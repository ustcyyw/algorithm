...
来源：力扣（LeetCode）
链接：https://algorithm.cn/problems/cycle-length-queries-in-a-tree
 */
class Solution {
public:
    vector<int> cycleLengthQueries(int n, vector<vector<int>>& queries) {
        ...
        for(auto& query : queries){
            int v = query[0], w = query[1];
            int dv = depth(v), dw = depth(w);
            if(dv > dw) {
                swap(v, w);
                swap(dv, dw);
            }
            for(int i = 0; i < dw - dv; i++)
                w /= 2;
            while (v != w) v /= 2, w /= 2;
            int dRoot = depth(v);
            res.push_back(dv + dw - dRoot - dRoot + 1);
        }
        return res;
    }
